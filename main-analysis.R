...
data <- data_1 # out of data_1, data_2, data_3
...
data_3$country_tier <- factor(data_3$country_tier)
data_3$country_tier <- relevel(data_3$country_tier, ref = "Country tier 4")
data_3$device_tier <- factor(data_3$device_tier)
data_3$device_tier <- relevel(data_3$device_tier, ref = "Android tier 5")

# regression to quantify treatment effect w/o background data
e3_offer_conversion <- lm(d30_offer_conversion ~ test_bucket,
                             data=data_3)
summary(e3_offer_conversion)

e3_offer_revenue <- lm(d30_offer_revenue ~ test_bucket,
                          data=data_3)
summary(e3_offer_revenue)

e3_conversion <- lm(d30_conversion ~ test_bucket,
                       data=data_3)
summary(e3_conversion)

e3_revenue <- lm(d30_revenue ~ test_bucket,
                    data=data_3)
summary(e3_revenue)

e3_rev_denoised <- lm(d30_rev_denoised ~ test_bucket,
                         data=data_3)
summary(e3_rev_denoised)

e3_repeat_purchases <- lm(d30_repeat_purchases ~ test_bucket,
                             data=data_3)
summary(e3_repeat_purchases)

e3_minutes <- lm(d30_minutes ~ test_bucket,
                    data=data_3)
summary(e3_minutes)

e3_rounds_played <- lm(d30_rounds_played ~ test_bucket,
                          data=data_3)
summary(e3_rounds_played)

e3_retention <- lm(d30_retention ~ test_bucket,
                      data=data_3)
summary(e3_retention)

e3_ad_views <- lm(d30_ad_views ~ test_bucket,
                     data=data_3)
summary(e3_ad_views)


#################################################################
##### exp. 3: qtes for pers. skimming effects in revenue #####
#################################################################

# set $4.99 starter pack condition as reference
data_3$test_bucket <- factor(data_3$test_bucket)
data_3$test_bucket <- relevel(data_3$test_bucket, ref = "Hold-out")

# create needed treatment dummy
data_3$pers_skim <- 0
data_3$pers_skim[data_3$test_bucket=="Personalized"] <- 1

# check values of quantiles in the respective joint distributions
quantile(data_3$d30_revenue, probs = .978)
quantile(data_3$d30_revenue, probs = .983)
quantile(data_3$d30_revenue, probs = .988)
quantile(data_3$d30_revenue, probs = .992)
