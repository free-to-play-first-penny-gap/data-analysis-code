##### non-parametric analysis of heterogeneous treatment effects in gdp per capita for experiment 3 #####
...
data_3$pers_skim <- as.factor(data_3$pers_skim)

# create 10k version of GDP per capita
data_3$gdp_p_capita_2017_10k <- data_3$gdp_p_capita_2017/10000
...
gam_model <- gam(d30_rev_denoised ~ s(gdp_p_capita_2017_10k, k = 8) + 
                   ...
                   pers_skim, 
                 data = data_3, method = "REML")

# define the range of the covariate for prediction
covariate_range <- seq(min(data_3$gdp_p_capita_2017_10k, na.rm = TRUE), 
                       max(data_3$gdp_p_capita_2017_10k, na.rm = TRUE), 
                       length.out = 100)
...
pred_treated <- predict(gam_model, 
                        newdata = data.frame(gdp_p_capita_2017_10k = covariate_range, 
                                             pers_skim = factor(1, levels = levels(data_3$pers_skim))),
                        type = "response", se.fit = TRUE)
pred_control <- predict(gam_model, 
                        newdata = data.frame(gdp_p_capita_2017_10k = covariate_range, 
                                             pers_skim = factor(0, levels = levels(data_3$pers_skim))),
                        type = "response", se.fit = TRUE)
